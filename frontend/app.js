...
function updateSearchUsage(remaining, limit) {
  ...
  if (num <= 0) {
    el.textContent = "Hoy no te quedan búsquedas gratuitas";
    bar.classList.add("exhausted");
  } else if (num <= 3) {
    el.textContent = "Te quedan " + num + " de " + lim + " búsquedas hoy";
...
function showShareBar() {
  ...
  bar.innerHTML = '<span style="font-size:13px;color:var(--text-secondary);white-space:nowrap">📤 Compartir resultados</span>' +
    ...
    '<button class="btn-icon" onclick="trackShare();copyShareLink()" title="Copiar enlace" style="color:var(--accent)">' +
